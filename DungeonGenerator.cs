...
public class DungeonGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        if (generateDungeonButton != null)
            generateDungeonButton.onClick.AddListener(() => { if (!isDungeonGenerated) StartCoroutine(GenerateDungeonOverTime()); });
    }

    IEnumerator GenerateDungeonOverTime()
    {
        if (isDungeonGenerated || dungeonPrefabs == null) yield break;
    ...
    {
        if (Vector2Int.Distance(pos, bossRoomCenter) < (bossRoomSize / 2 + bossRoomTunnelBuffer + minNodeDistance)) return false;
    ...
    void PrepareObjectsForBuilding()
    {
        foreach (var pos in floorPositions)
            ...
            {
                pos = pos,
                type = bossRoomPositions.Contains(pos) ? PrefabType.BossFloor :
        ...
        if (spawnInBossRoom) buildQueue.Add(new BuildItem { pos = bossRoomCenter, type = PrefabType.Spawner });
    }
